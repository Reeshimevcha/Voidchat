/**
 * VoidChat v6 — Fixed permission model:
 * - Every joined user reads by default (no owner gatekeeping)
 * - Each SENDER controls who sees their own messages (myVisibleTo)
 * - Owner ACCESS CONTROL = download permission + kick only
 * - Download ONLY shows messages sender allowed you to see
 * - Voice notes via base64 (no Firebase Storage)
 */
...
async function handleCreate(){
  ...
  // Owner: canDownload=true. Everyone reads by default — no canRead gatekeeping.
...
async function handleJoin(){
  ...
  // Every joined user can read — sender controls visibility via myVisibleTo
...
function enterChat(){
  ...
  // ACCESS CONTROL only useful in group for owner (download permissions)
...
function renderUsers(users){
  ...
  for(const[uid,u]of Object.entries(users)){
    ...
    li.innerHTML=`
      ...
      <div class="um">
        <span class="un">${u.name||'?'}${uid===S.uid?` <span class="you-tag">YOU</span>`:''}${ownerTag}</span>
...
// ── Owner Access Control (GROUP only) — controls download + kick ───────────
// Reading is NOT owner-controlled. Each sender controls visibility themselves.
...
// ── MY PRIVACY — sender controls who sees THEIR messages ──────────────────
...
// ── canView: ONLY checks sender's myVisibleTo — owner has NO special power ─
function canView(data){
  // Sender always sees own messages
  if(data.from===S.uid) return true;
  // If sender set visibleTo list and current user is NOT in it → cannot view
  if(data.visibleTo && !data.visibleTo.includes(S.uid)) return false;
  // Otherwise everyone who joined can read
  return true;
}
...
async function handleSend(){
  ...
  try{
    if(S.schedTime){
      ...
      await msgRef.set({from:S.uid,fromName:S.name,timestamp:Date.now(),
...
function scheduleDelivery(schedId,data){
  ...
  S.schedTimers[schedId]=setTimeout(async()=>{
    ...
    await msgRef.set({from:data.from,fromName:data.fromName,timestamp:Date.now(),
...
// ── Text message ───────────────────────────────────────────────────────────
// No access → show encrypted emojis (sender hid from you)
// Has access → show encrypted emojis + REVEAL button
function buildTextMsg(id,data,isMine,canSee){
  if(!data.content) return '';
  if(!canSee){
    // Show encrypted emojis — user knows a message exists but can't decrypt
    return `<div class="msg-content msg-encrypted-only" title="Sender has restricted visibility">${EmojiCipher.shortDisplay(data.content)}</div>`;
  }
...
function buildImageMsg(data){
  ...
  ${data.fileName?`<div class="img-caption">${data.fileName}</div>`:''}</div>`;
}
...
// ── Download — SECURE: only includes messages sender allowed you to see ────
async function downloadChat(){
  ...
  let rows='';
  for(const m of msgs){
    // SECURITY: skip messages sender hid from this user
