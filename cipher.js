/**
 * VoidChat Cipher v4 — Short display + per-message rotation
 * Visual output: first 6 emojis + count badge (e.g. 🐶🌸😀🎭🦊🍕 +24)
 */
const EmojiCipher = (() => {
  const BASE = (() => {
  ...
  // Short display: first 6 emojis + count
  const shortDisplay=emojiStr=>{
    const all=segs(emojiStr);
    if(all.length<=8)return emojiStr;
    return all.slice(0,6).join('')+` +${all.length-6}🔒`;
  };

  return{encrypt,decrypt,shortDisplay};
})();
